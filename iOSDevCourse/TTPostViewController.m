...
@interface TTPostViewController () <UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UITextViewDelegate,TTImageViewGalleryDelegete,TTAddPostDelegete>
...
- (void)updateData {
    ...
    [[TTServerManager sharedManager]getCommentById:self.wallPost.post_id ownerID:self.wallPost.owner_id count:MAX(20, [self.comentPostArray count]) offset:0 onSuccess:^(NSArray *wallComment) {
        ...
        if ([wallComment count] > 0) {
            ...
            for (int i = (int)[self.comentPostArray count] - (int)[wallComment count]; i < [self.comentPostArray count]; i++) {
                ...
                CGSize newSize = [self setFramesToImageViews:nil imageFrames:[[self.comentPostArray objectAtIndex:i] attachment] toFitSize:CGSizeMake(302, 400)];
...
- (void)getWallPostCommentFromServer {
    ...
    [[TTServerManager sharedManager]getCommentById:self.wallPost.post_id ownerID:self.wallPost.owner_id count:20 offset:[self.comentPostArray count] onSuccess:^(NSArray *wallComment) {
...
- (void)addLike:(id)sender {
    ...
    TTComment *comment = [self.comentPostArray objectAtIndex:indexPath.row];
    
    if (comment.can_like) {
        [[TTServerManager sharedManager]postLikeOnWall:self.wallPost.owner_id inPost:comment.coment_id type:@"comment" onSuccess:^(NSDictionary *result) {
        ...
        [[TTServerManager sharedManager]postDeleteLikeOnWall:self.wallPost.owner_id inPost:comment.coment_id  type:@"comment" onSuccess:^(NSDictionary *result) {
...
- (void)addComment:(UIBarButtonItem *)sender {
    ...
    TTAddPostViewController *vc = [[TTAddPostViewController alloc]initWithTypePost:TTPostTypeWallComment];
    vc.delegate = self;
    vc.data = self.wallPost;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *postIdentifier = @"detailPostCell";
